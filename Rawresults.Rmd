...
load("C:/Users/Jorge/Dropbox/Dimensions Data/BetaDistEnv.RData")
head(BetaEnvDist)
```
...
OriginalRow is the row number in the siteXspp matrix made by gabriel. When i say To and From, i mean a comparison of cell To to cell From, think of it as a direction. Cell 'To' is localted at *To.xcord* (x coordinate), *To.ycord* on the earth's surface, and cell 'From' on *From.xcord* and *From.ycord*. The *envdist* is the Euclidean distance of the PCA of the first 19 bioclim variables. *Km* is the direct Euclidean distance on the Earth's surface. *Betasim* is the phylogenetic betadiveristy, *Sorenson* the taxonomic betadiversity, and *MNTD* (mean nearest taxon distance) is the trait betadiversity.
...
system.time(row1<-BetaEnvDist[J(1,2)])

#Asking the column directly is slightly slower (still fast)
system.time(row1<-BetaEnvDist[To.OriginalRow %in% "1"])

#Asking for two columns, since cell 8 could be in To or From
system.time(row1<-BetaEnvDist[To.OriginalRow %in% "1"|From.OriginalRow %in% "1"])
head(row1)
```

Check out how fast that was remember BetaEnvDist has `r{dim(BetaEnvDist)}` rows, and i'm just on my old desktop.
