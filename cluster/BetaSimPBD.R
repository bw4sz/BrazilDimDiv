...
matpsim <- function(phyl, com) # make sure nodes are labelled and that com and phyl species match
{
 ...
 id<- get.jid(length(spp))

  brs <-  pbdLapply(id,spp,function(i){ #this loop makes a list of branches for each species 
comm.print(i) 	
...
comm.print(dim(brs))
  ...
  tcellbr <- pbdLapply(rownames(com),function(j){cellbr(j,spp_br,com)})
  ...
  psim <- pbdLapply(rownames(tcellbr),function(j){
    print(j);cell_a <- j; 
    unlist(
      lapply(
